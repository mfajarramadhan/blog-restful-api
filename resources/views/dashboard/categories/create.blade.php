    ...
    <div class="flex flex-col w-full p-8 mx-auto mt-4 bg-white border rounded-lg">
        <form action="/dashboard/categories" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="slug" name="slug" readonly class="read-only:bg-gray-200 w-full rounded border border-gray-300 bg-white py-1 px-3 text-base leading-8 text-gray-700 outline-none transition-colors duration-200 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 @error('slug') valid:border-red-600 valid:focus:border-red-600 valid:focus:ring-red-200 @enderror" required value="{{ old('slug') }}"/>
                @error('slug')
                    <div class="text-xs text-red-600">
                      {{ $message }}
                    </div>
            ...
            <div class="mb-5">
                <label for="color" class="text-sm leading-7 text-gray-600">Color</label>
                <select class="block w-full text-sm font-medium transition duration-75 border border-gray-800 rounded-lg shadow-sm h-9 focus:border-blue-600 focus:ring-1 focus:ring-inset focus:ring-blue-600 bg-none" name="color">
                    @foreach ($categories as $category)
                    <option value="{{ $category->color }}" {{ old('category_id') == $category->color ? 'selected': ''}}>{{ $category->color }}</option>
                    @endforeach
                    <option value="orange" {{ old('color') == $category->color ? 'selected': ''}}>orange</option>
                    <option value="purple" {{ old('color') == $category->color ? 'selected': ''}}>purple</option>
                    <option value="teal" {{ old('color') == $category->color ? 'selected': ''}}>teal</option>
                </select>
            </div>
            {{-- <div class="mb-4">
